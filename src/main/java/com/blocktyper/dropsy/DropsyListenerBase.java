...
public class DropsyListenerBase extends BlockTyperListener {
    ...
    protected boolean spell(Location location, Key spellsRoot, String spellKey, Key parentRoot,
                            List<String> spellsFromRandomDistribution, Player player, double materialSpellChance) {
        ...
        return attemptSpell(location, spellsRoot, spellKey, forceSpell, materialSpellChance, player);
    }
    ...
    private ItemStack getDropItem(String dropMaterialName, Key dropRoot) {
    ...
    protected boolean attemptDrop(Location location, Key dropsRoot, String dropKey,
        ...
        ItemStack dropItem = null;
        boolean isCustomMaterialDrop = getConfig().contains(dropRoot.end(Config.MATERIAL));
        boolean isCustomRecipeDrop = getConfig().contains(dropRoot.end(Config.RECIPE));

        if (isCustomMaterialDrop) {
            dropItem = getDropItem(getConfig().getString(dropRoot.end(Config.MATERIAL)), dropRoot);
        } else if (isCustomRecipeDrop) {
            ...
            dropItem = getDropItem(dropKey, dropRoot);
        }
        ...
        if (getConfig().contains(dropRoot.end(Config.NAME))) {
            String customName = getConfig().getString(dropRoot.end(Config.NAME));
        ...
        if (getConfig().contains(dropRoot.end(Config.LORE))) {
            List<String> lore = getConfig().getStringList(dropRoot.end(Config.LORE));
            ItemMeta itemMeta = dropItem.getItemMeta();
            if (itemMeta != null) {
        ...
        if (getConfig().contains(dropRoot.end(Config.ENCHANTMENTS))) {
            ConfigurationSection enchantments = getConfig().getConfigurationSection(dropRoot.end(Config.ENCHANTMENTS));
            if (enchantments != null && enchantments.getKeys(false) != null) {
                for (String enchantmentName : enchantments.getKeys(false)) {
                    ...
                    Key enchantmentKey = new Key(dropRoot.end(Config.ENCHANTMENTS)).__(enchantmentName);
                    if (getConfig().contains(enchantmentKey.end(Config.LEVEL))) {
                        level = getConfig().getInt(enchantmentKey.end(Config.LEVEL));
                    }
    ...
    private boolean isInRange(Integer highestY, Integer lowestY, Location location) {
    ...
    private boolean shouldItemBeDropped(Key dropRoot, Location location, Player player, boolean forceDrop, double materialDropChance) {
        ...
        if (!isInRange(highestY, lowestY, location)) {
            return false;
        }
    ...
    protected boolean attemptSpell(Location location, Key spellsRoot, String spellKey,
                                   boolean forceSpell, double materialSpellChance, Player player) {
        ...
        if (getConfig().contains(spellRoot.end(Config.COMMAND))) {
            ...
            debugInfo("COMMAND SEQUENCE: " + commandSequenceRoot.end(Config.COMMANDS));
        ...
        if (commands != null && !commands.isEmpty()) {
            return executeCommands(commands, messages, variablesMap, player);
        }

        return false;
    }

    protected boolean executeCommands(List<String> commands, List<String> messages, Map<String, String> variablesMap, Player player) {
        ...
        for (String command : commands) {
            ...
            if (variablesMap != null && !variablesMap.isEmpty()) {
                for (String variable : variablesMap.keySet()) {
                    debugInfo("Replacing variable in command: " + variable + "=" + variablesMap.get(variable));
        ...
        if (messages != null) {
            for (String message : messages) {
                ...
                if (variablesMap != null && !variablesMap.isEmpty()) {
                    ...
                    if(variablesMap.containsKey(Config.SOURCE) && variablesMap.get(Config.SOURCE) != null) {
                        messageToSend = messageToSend.replace("SOURCE", variablesMap.get(Config.SOURCE));
                    }
                }
    ...
    protected RegionsToProcess getRegionsToProcess(String world, int x, int z) {
        ...
        for (String region : getConfig().getConfigurationSection(regionsRoot.getVal()).getKeys(false)) {
            Key regionRoot = new Key(regionsRoot.end(region));

            debugInfo("Region root: " + regionRoot.getVal());

            boolean isDefaultRegion = !getConfig().contains(regionRoot.end(Config.BOUNDS));

            if (getConfig().contains(regionRoot.end(Config.WORLDS))) {
            ...
            Key boundsRoot = new Key(regionRoot.getVal()).__(Config.BOUNDS);
            ...
            if (highestX != null && x > highestX) {
                debugInfo("X too high (" + x + ">" + highestX + ")");
                continue;
            }
            if (lowestX != null && x < lowestX) {
                debugInfo("X too low (" + x + "<" + lowestX + ")");
                continue;
            }

            if (highestZ != null && z > highestZ) {
                debugInfo("Z too high (" + z + ">" + highestZ + ")");
                continue;
            }
            if (lowestZ != null && z < lowestZ) {
                debugInfo("Z too low (" + z + "<" + lowestZ + ")");
                continue;
            }
            ...
            if (priority == null) {
                ...
                debugInfo("Lower priority");
                continue;
            } else if (tempPriority < priority) {
                debugInfo("Clearing lower priorities");
                priorityRegions.clear();
            }

            debugInfo("Adding priority region: " + regionRoot.getVal());
